...
当我们执行完成返回之后 schemaMappings中就有值了，这些值是在什么时候读进去的？
...
**如果发生循环依赖的对象，不需要代理的话，只需要二级缓存足以解决所有问题，但是当代理指挥就无法解决了，必须要使用三级缓存来解决。**
...
![image-20230414194900652](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230414194900652.png)
 ...
 ![image-20230420084807269](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420084807269.png)



![image-20230420084958913](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420084958913.png)



进入到populateBean的时候，BeanWrapper中的A对象的b属性为null，没有做任何的一个赋值操作

![image-20230420085914693](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420085914693.png)

![image-20230420090004702](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420090004702.png)



![image-20230420090043078](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420090043078.png)

![image-20230420090107510](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420090107510.png)

拿b对象

![image-20230420090128266](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420090128266.png)



![image-20230420091007762](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420091007762.png) 

执行到b对象的时候，当前bean容器中会存在两个b对象，一个是代理对象，一个是包含属性a的b对象。

![image-20230420090954976](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420090954976.png)

执行addSingleton方法的时候会把b对象从二级缓存中移除，把cglibB放到一级缓存中。

![image-20230420091230715](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420091230715.png)

当缓存中没有的时候，就会往回找，这个时候我们会发现我们使用的b是动态代理的对象，当我们有了动态代理的b对象之后，需要最终给对象a完成一个赋值的操作。

![image-20230420091407380](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420091407380.png)



![image-20230420091517837](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420091517837.png) 



报错

![image-20230420091622785](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20230420091622785.png)
...
是会存在两个，但是三级缓存中只会放一个，一开始确实会创建一个普通的A对象，但是生成代理的时候会把A对象给覆盖掉，而且三级缓存里面不会既存在A的普通对象和A的代理对象，只会有一个。A的实际对象其实是被弃用掉了。
