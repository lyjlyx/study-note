...
select a.*, b.s_score 01score, c.s_score 02score from student a 
inner join score b
on a.s_id = b.s_id and b.c_id = '01'
inner join score c
on c.s_id = b.s_id and c.c_id = '02'
...
-- 3、查询平均成绩大于等于60分的同学的学生编号和学生姓名和平均成绩
select a.*, avg(b.s_score) avg_score from student a
left join score b
on a.s_id = b.s_id
group by a.s_id, a.s_name having avg(b.s_score) >= 60

-- 5、查询所有同学的学生编号、学生姓名、选课总数、所有课程的总成绩
select a.*, sum(b.s_score) sum , count(b.c_id) count from student a
left join score b
on a.s_id = b.s_id 
group by a.s_id having sum(b.s_score) and count(b.c_id)
...
FROM
	student a where 
a.s_id in (select s_id from score b where b.c_id = '01') and a.s_id not in (select s_id from score c where c.c_id = '02')

-- 11、查询没有学全所有课程的同学的信息
-- 要查询没有学过所有课程的同学，即查询出所学课程数小于总课程数的学生即可。
select student.*
from student  where student.s_id in
	(select st.s_id
	from student st left join score as sc on st.s_id=sc.s_id
	group by st.s_id
...
where s.s_id not in(
	...
	inner join score d on a.s_id = d.s_id and d.c_id = '03'
)

-- 12、查询至少有一门课与学号为"01"的同学所学相同的同学的信息 
select s.* from student s
where  s.s_id in (
select c.s_id from  score c where c.s_id ='01')
