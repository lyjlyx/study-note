...
根据递归的方式将一个一个数组按照上面的方式递归排好

![image-20240823131006550](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20240823131006550.png)



在arr[L...R]的范围上，拿arr[R]做划分值p，在L...R整个范围上 <p的在左边 =p的在中间 >p的在右边，递归下去数组总有排好的时候。

返回一个数组，数组中是=于区的左边界和右边界两个数组成的小数组，表示等于区从哪到哪。

![image-20240823131715721](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20240823131715721.png)

```java
	//在arr[L...R]的范围上，拿arr[R]做划分值p，
    // 在L...R整个范围上 <p的在左边 =p的在中间 >p的在右边
    // 返回一个数组，数组中是=于区的左边界和右边界
    public int[] partition(int[] arr, int L, int R) {
        ...
        while (index < moreL) {
            if (arr[index] < arr[R]) {
                // 小于区和当前数做交换，小于区跳下一个，当前数++
                swap(arr, ++lessR, index ++);
            } else if (arr[index] > arr[R]) {
                // 大于区的前一个数和当前数做交换，大于区往前进也就是--
                swap(arr, -- moreL, index);
            } else {
                index ++;
            }
...
因为最后的数划分值，要跟大于区的第一个数做交换的，这样才算排好，所以等于区的右边界就是moreL，而左边界就是小于区的右边界再加1。

![image-20240825161528016](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20240825161528016.png)
...
![image-20240825162356594](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20240825162356594.png)
 ...
 * 递归和非递归实现PartitionAndQuickSort
 */
public class RecursionPartitionAndQuickSort {
    ...
    public static void main(String[] args) {
        int[] arr = {7, 1, 3, 5, 4, 5, 9, 18, 5, 1, 14, 22, 44, 2, 4};
...
![image-20240825164145390](https://lyx-study-note-image.oss-cn-shenzhen.aliyuncs.com/img/image-20240825164145390.png)

如上图例子：最大的任务是从0...100 **中间的位置占住之后**左边下发的任务就是0...30开始排序，右边的任务就是70...100开始排序

等于划分值的区域是肯定有的，但是有可能会没有小于划分值或者大于划分值的区域，
 ...
 * 非递归快排
 */
public class NoRecursionPartitionQuickSort {
    // 下发一个任务
    ...
    public static void quickSort(int[] arr) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (equals[0] > cur.L) {// 判断有没有小于区域
                // 等于区域的第一个位置只有大于当前范围的L才有小于区域,才有子任务
                // 往里丢子任务   当前L到等于区的前一个位置，才是小于区域的范围
                stack.push(new Job(cur.L, equals[0] - 1));
            }

            if (equals[1] < cur.R) { // 等于区域的最后一个位置要小于当前数组的R才有大于区
                // 才有右边的子任务
                // 等于区域的右边位置+1 到当前位置的R
    ...
    public static void main(String[] args) {
        int[] arr = {71, 12, 3, 5, 4, 5, 9, 18, 5, 1, 14, 22, 44, 2, 4};
        quickSort(arr);
        for (int i = 0; i < arr.length; i++) {
            System.out.print(arr[i] + " ");
        }
